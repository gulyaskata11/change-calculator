import { Input, Stack, Box, Button, Checkbox, CheckboxGroup, HStack } from "@chakra-ui/react";
...
const CalculatorForm = () => {
  ...
  const onClickSubmit = () => {
    const writeResult = calculateResult(amountOfMoney, amountToBePaid);
  ...
  const handleCurrentValuta = (array) => {
    ...
    console.log(currentValuta)
  }

  return (
    <Box p={10}>
      <Stack spacing={4} align="center">
        <CheckboxGroup colorScheme="green" value={currentValuta} onChange={handleCurrentValuta}>
          <HStack>
            {valuta.map((value) => <Checkbox key={value} value={value}>{value}</Checkbox>)}
          </HStack>
        </CheckboxGroup>
        <Input
